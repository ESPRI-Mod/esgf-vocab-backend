...
def instantiate_from_json(
    json_obj: list | dict | str,
) -> str | DataDescriptor | tuple | list | ProjectSpecs | Item | MatchingTerm | None:
    ...
    if select:
        ...
        if isinstance(json_result, list):
            ...
            if isinstance(item, list):
                ...
                term_dict = item[-1]
                print(
                    f"DEBUG: Cross case - item[-1] = {term_dict}, len = {len(term_dict)}, type = {term_type}, is_plain_term = {is_plain_term}"
                )
                ...
                print(
                    f"DEBUG: Single item case - item = {item}, type = {type(item)}, len = {len(item) if hasattr(item, '__len__') else 'N/A'}, term_type = {term_type}, is_plain_term = {is_plain_term}"
                )
                assert "id" in item
